 ...
 */
 ...
 /* Still under construction right now i've just adjusted the sensor to show open much quicker.
 *  In order to achieve my goal I will need to make some bigger changes that either adjust the 
 *  closed state dynamically or create a separate device handler for each sensor. The raw value
 *  when closed varies too much between devices due to mounting angles and vairation in door. 
 *  Therefore in order to set the sensor to be as sensitive as I want, it may be outside the range
 *  for another sensor that is actually still closed. 
 */
...
private List<Map> handleAcceleration(descMap) {
	...
	if (descMap.clusterInt == 0xFC02 && descMap.attrInt == 0x0010) {
        ...
        log.debug "initiate counter to zero"
       	state.counter = 0
		result << [
				name           : "acceleration",
				value          : value,
...
private List<Map> parseAxis(List<Map> attrData) {
	...
	if (garageSensor == "Yes" && state.counter > 0) {
		results += garageEvent(xyzResults.z)
        log.debug "sensor state checked"
	}
    state.counter = 1
    log.debug "counter is $state.counter"
...
def configure() {
	...
	// create a state variable to track number of times the xyz has been parsed
    state.counter = 0
    log.debug "counter initiated"
